...
int main(int argc, char* argv[])
{
   ...
   if (argc != 7)
   {
      cout << "usage: sendfile server_port use_blast(0 or 1) udt_sendbuff udp_sendbuff mss blast_rate"  << endl;
      return 0;
   }
   ...
   UDTSOCKET serv = UDT::socket(res->ai_family, res->ai_socktype, res->ai_protocol);
    ...
    int udt_sendbuff = atoi(argv[3]);
    int udp_sendbuff = atoi(argv[4]);
    int mss = atoi(argv[5]);
    int blast_rate = atoi(argv[6]);

    // no im not crazy the server sends
   // UDT Options
   if (blast)
   ...
   freeaddrinfo(res);
    ...
    // maybe we need to set this after ::accept
   // using CC method
   if (blast) {
